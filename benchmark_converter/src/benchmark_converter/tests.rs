...
fn convert() {
    ...
    let input_1 = serde_json::from_str::<benchmark_converter::InputReport>(foundry_report_1)
    ...
    let input_2 = serde_json::from_str::<benchmark_converter::InputReport>(foundry_report_2)
        .expect("Failed to parse foundry report");
    ...
    let contents = match output {
        benchmark_converter::Output::SingleFile(file) => file,
        benchmark_converter::Output::MultipleFiles(mut files) => files.remove(0).content,
        _ => unimplemented!(),
    };
