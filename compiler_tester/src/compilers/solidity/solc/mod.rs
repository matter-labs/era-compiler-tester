...
impl SolidityCompiler {
    ...
    pub fn standard_json_output(
        ...
        let output_selection = match mode {
            ...
            Mode::YulUpstream(_mode) => SolcStandardJsonInputSelection::new_required(Some(
    ...
    fn compile_for_eravm(
        ...
        for (file, contracts) in contracts.into_iter() {
            for (name, contract) in contracts.into_iter() {
                ...
                let mut build = era_compiler_llvm_context::EraVMBuild::new(bytecode, vec![], None);
                build.bytecode_hash = Some(bytecode_hash);
